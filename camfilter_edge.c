...
int minCont = 20, maxCont = 255, dilate_pos = 1;
int cenH=30, radH=19, minS=0, maxS=255, minV=50, maxV=255;
int low_thres = 20, high_thres = 170, sobel = 3;

// Load the source image. HighGUI use.
IplImage *image01 = 0, *image03 = 0, *image04 = 0;
IplImage *imH = 0, *imS = 0, *imV = 0, *imHSV = 0, *imFil = 0, *imCont = 0;
IplImage *imDeriv = 0, *imDerivU8 = 0, *imDerivU8c = 0;
...
int main( int argc, char** argv )
{
  ...
  cvCreateTrackbar( "Dilate", "Result", &dilate_pos, 100, NULL);
  cvCreateTrackbar( "low threshold", "Result", &low_thres, 255, NULL);
  cvCreateTrackbar( "high threshold", "Result", &high_thres, 255, NULL);
  ...
  imDeriv = cvCreateImage(cvSize(image01->width,image01->height), IPL_DEPTH_16S, 1);
  ...
  imDerivU8 = cvCloneImage(image03); cvZero(imDerivU8);
  imDerivU8c = cvCloneImage(image03); cvZero(imDerivU8c);
  ...
  while (!quit)
  {
    ...
    cvShowImage("Derived", imDerivU8c);
...
void process_image(int h)
{
    ...
    // filter image to find contours
    cvCanny(image03, imDerivU8, low_thres, high_thres, sobel);

    // clean image point sequences
    cvDilate( imDerivU8, imDerivU8, NULL, dilate_pos);

    // display image ( find contour destroy the image )
    imDerivU8c = cvCloneImage( imDerivU8 );

    // Create dynamic memory storage and sequence. 
    stor = cvCreateMemStorage(0);
    cont = cvCreateSeq(CV_SEQ_ELTYPE_POINT, sizeof(CvSeq), sizeof(CvPoint) , stor);

    cvFindContours( imDerivU8, stor, &cont, sizeof(CvContour), 
